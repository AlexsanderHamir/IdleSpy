...
func (p *Pipeline) GetPipelineStats() map[tracker.GoroutineId]*tracker.GoroutineStats {
...
func generateData(ctx context.Context, count int, r *rand.Rand, stats *tracker.GoroutineManager) <-chan Data {
	...
	go func() {
		...
		for i := range count {
			...
			select {
			case <-ctx.Done():
				stats.TrackSelectCase("context_done_generateData", time.Since(startTime))
				return
			case output <- Data{
				ID:        i,
				Value:     r.Float64() * 100,
				...
				stats.TrackSelectCase("data_send_generateData", time.Since(startTime))
